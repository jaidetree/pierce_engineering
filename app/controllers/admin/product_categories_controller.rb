class Admin::ProductCategoriesController < ApplicationController
	...
	def create
		...
		respond_to do |format|
			if @admin_product_category.save
				if rifle_page? 
					format.html { redirect_to( new_admin_rifle_category_path, :notice => 'Rifle category was successfully created.') }
					format.xml  { render :xml => @admin_product_category, :status => :created, :location => @admin_product_category }
				else
					format.html { render :action => "new", :notice => 'Product category was successfully created.' }
					format.xml  { render :xml => @admin_product_category, :status => :created, :location => @admin_product_category }
				end
			else
				if rifle_page? 
	...
	private
	def setup
		@product_type = ProductsLib.setup( admin_rifle_categories_path, request.fullpath )
	end
end
